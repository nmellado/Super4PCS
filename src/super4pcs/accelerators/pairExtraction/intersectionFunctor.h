    ...
    )
{
  ...
  epsilon = 1.f/pow(2,lvlMax);
  
  //std::cout << "Compute intersection for esp=" << epsilon << std::endl;
...
//#pragma omp critical
            earlyNodes.push_back(std::pair<Node, Scalar>(n, edgeHalfLength+epsilon));
          }
          break;
        }
      }
